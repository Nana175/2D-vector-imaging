...
class MatDataset(Dataset):
    def __init__(self, input_dir, target_dir, transform):
        ...
        target_dir: ターゲットデータのディレクトリ（例：'data/outputs'）
        transform: オプションの前処理
        """
...
class MatDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        train_target_dir = os.path.join(self.train_dir, "outputs")
        val_input_dir = os.path.join(self.val_dir, "inputs")
        val_target_dir = os.path.join(self.val_dir, "outputs")
        test_input_dir = os.path.join(self.test_dir, "inputs")
        test_target_dir = os.path.join(self.test_dir, "outputs")
